...
const Navbar: React.FC<NavbarProps> = ({ currentPath, setCurrentPath }) => {
  ...
  const isAdmin =
    user?.email &&
    ['admin@erthaloka.com', 'rama@erthaloka.com', 'erthaloka@gmail.com'].includes(user.email);
  ...
  return (
    ...
    >
      <div className="relative container mx-auto px-6 py-4 flex items-center justify-between">
                ...
                {showUserMenu && (
                  <div className="absolute right-0 mt-2 w-48 bg-gray-800 rounded-lg shadow-lg border border-gray-700 z-50">
                    ...
                    <button
                      ...
                      className="w-full text-left px-4 py-2 text-red-300 hover:text-red-200 hover:bg-gray-700 transition-colors"
                    >
                      Logout
                    </button>
                  </div>
        ...
        <div
          className={`absolute top-full left-0 w-full origin-top transform transition-transform duration-300 ease-in-out z-40 ${
            isOpen ? 'scale-y-100 opacity-100' : 'scale-y-0 opacity-0 pointer-events-none'
          } bg-black overflow-hidden rounded-b-lg`}
        >
          <div className="text-white px-6 pt-4 pb-6 space-y-4">
            {navItems.map((item) => (
              <button
                key={item.label}
                onClick={() => handleNavClick(item.path)}
